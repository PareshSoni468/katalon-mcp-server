...
import * as xml2js from 'xml2js'; // Convert XML to JavaScript objects
import { glob } from 'glob'; // Find files using patterns

/**
 * 🎯 Object Repository Item Interface
 * Represents a single UI element (button, text field, etc.) that tests can interact with
 * Think of this as a detailed profile for each clickable/testable element on your website
 */
export interface ObjectRepositoryItem {
  id: string; // Unique identifier within Katalon
  name: string; // Human-readable name (e.g., "Login Button")
  description: string; // What this element does or represents
  tag: string; // HTML tag type (button, input, div, etc.)
  selector: ObjectSelector; // Primary way to find this element on the page
  smartLocators?: SmartLocator[]; // AI-powered alternative ways to find the element
  healingData?: HealingData; // Information about self-repair when element changes
  metadata?: ObjectMetadata; // Additional tracking information
}

/**
 * 🔍 Object Selector Interface
 * The specific method used to locate an element on a web page
 * Like GPS coordinates for finding elements in your application
 */
export interface ObjectSelector {
  type: 'xpath' | 'css' | 'id' | 'name' | 'class' | 'tag' | 'link_text' | 'partial_link_text'; // Strategy to find element
  value: string; // The actual selector text/path
  priority: number; // How preferred this selector is (1 = highest priority)
}

/**
 * 🧠 Smart Locator Interface
 * AI-powered alternative ways to find elements when primary selectors fail
 * These are backup plans generated by machine learning
 */
export interface SmartLocator {
  type: string; // Type of smart locator strategy
  value: string; // The smart locator value
  confidence: number; // How confident the AI is (0-1, where 1 = very confident)
  isActive: boolean; // Whether this smart locator is currently being used
}

/**
 * 🔧 Healing Data Interface
 * Information about how an element has been automatically repaired when it breaks
 * Like a medical record for your test elements
 */
export interface HealingData {
  healedCount: number; // How many times this element has been auto-fixed
  lastHealed: Date; // When it was last repaired
  originalSelector: ObjectSelector; // The original way to find this element
  healedSelectors: ObjectSelector[]; // New ways discovered to find the element
  isAutoHealingEnabled: boolean; // Whether auto-repair is turned on for this element
}

/**
 * 📋 Object Metadata Interface
 * Additional tracking and usage information for repository items
 * Like a usage report and history log for each element
 */
export interface ObjectMetadata {
  createdDate: Date; // When this element was first added
  lastModified: Date; // When it was last updated
  usageCount: number; // How many times tests have used this element
  associatedTestCases: string[]; // Which test cases use this element
}

/**
 * ⚙️ Object Management Options Interface
 * Configuration for performing operations on repository items
...
export interface ObjectManagementOptions {
  ...
  action: 'create' | 'update' | 'delete' | 'heal' | 'analyze'; // What operation to perform
  objectName?: string; // Name of the object to work with (if applicable)
  selector?: string; // New selector value (for create/update operations)
  selectorType?: string; // Type of selector (xpath, css, id, etc.)
  description?: string; // Description for the object (for create/update)
  smartHealingEnabled?: boolean; // Whether to enable smart healing for this object
}
...
export class KatalonObjectRepository {
  ...
  async manageObjects(args: any): Promise<{ content: any[] }> {
    ...
    try {
      ...
      throw new Error(
        `Object repository operation failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
...
${analysis.categories.map((category: any) => `- **${category.name}**: ${category.count} objects`).join('\n')}`;
  ...
  private async performRepositoryAnalysis(objectRepoPath: string): Promise<any> {
    ...
    return {
      ...
      avgSelectorsPerObject:
        Object.values(selectorTypeCounts).reduce((a, b) => a + b, 0) / totalObjects,
