...
export class KatalonKeywordManager {
  ...
  async manageKeywords(args: any): Promise<{ content: any[] }> {
    ...
    try {
      ...
      throw new Error(
        `Keyword management failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
...
**File**: ${keywordPath}`;
  ...
  private generateCustomKeywordContent(keyword: {
    ...
    const parameterList = keyword.parameters.map(p => `${p.name}`).join(', ');
  ...
  private getBuiltInKeywords(): KeywordInfo[] {
    return [
      // WebUI Keywords
      {
        ...
        examples: ['WebUI.openBrowser("https://example.com")', 'WebUI.openBrowser("")'],
      },
      {
        name: 'WebUI.setText',
        ...
        parameters: [
          {
            name: 'request',
            type: 'RequestObject',
            description: 'Request object to send',
            required: true,
          },
        ],
      ...
      // Mobile Keywords
      {
        ...
        parameters: [
          ...
          {
            name: 'uninstallAfterCloseApp',
            type: 'boolean',
            description: 'Uninstall app after closing',
            required: false,
          },
        ],
        returnType: 'void',
